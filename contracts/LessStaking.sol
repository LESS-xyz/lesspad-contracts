...
contract LessStaking is Ownable {
	...
	uint24 constant public SEC_IN_DAY = 86400;
	...
	function stake(uint256 lpAmount, uint256 lessAmount) external {
		...
		if (lpAmount > 0) {
			require(lpToken.transferFrom(_msgSender(), address(this), lpAmount), "Error: LP token tranfer failed");
		}
		if (lessAmount > 0) {
			require(lessToken.transferFrom(_msgSender(), address(this), lessAmount), "Error: Less token tranfer failed");
		}
		...
		stakeList[_msgSender()].push(StakeItem(stakeIdLast, block.timestamp, lpAmount, lessAmount, 0, 0));
	...
	function _unstake(
		...
		require(lpAmount <= deposit.stakedLp , "Error: insufficient LP token balance");
		require(lessAmount <= deposit.stakedLess, "Error: insufficient Less token balance");
		require(lpRewardsAmount <= deposit.lpEarned , "Error: insufficient LP token rewards");
		require(lessRewardsAmount <= deposit.lessEarned, "Error: insufficient Less token rewards");

		uint256 unstakedLp = lpAmount;
		uint256 unstakedLess = lessAmount;
		bool isUnstakedEarlier = block.timestamp.sub(deposit.startTime) < minDaysStake.mul(SEC_IN_DAY);
		...
		require(lpToken.transfer(staker, tranferedLp), "Error: LP transfer failed");
		require(lessToken.transfer(staker, tranferedLess), "Error: Less transfer failed");
	...
	function distributePenalty(uint256 lp, uint256 less) view internal {
	...
	function getLpBalanceByAddress(address staker) view public returns(uint256){
	 ...
	 * @param staker staker address
	 */
	function getLessBalanceByAddress(address staker) view public returns(uint256){
	 ...
	 * @param staker staker address
	 */
	function getOverallBalanceInLessByAddress(address staker) view public returns(uint256){
	 ...
	 */
	function getLpInLess(uint256 _amount) view public returns(uint256) {
	 ...
	 */
	function getOverallLP() view public returns(uint256){
		return allLp;
	}
	 ...
	 */
	function getOverallLess() view public returns(uint256){
		return allLess;
	}
	/**
	 * @dev return full contract balance converted in Less
	 */
	function getOverallBalanceInLess() view public returns(uint256){
	...
	function _getBalanceByAddress(address staker, BalanceType balanceType) view internal returns (uint256 balance){
		...
		if (deposits.length > 0) {
			for (uint256 i = 0; i < deposits.length; i++) {
				if (balanceType == BalanceType.Lp) balance = balance.add(deposits[i].stakedLp);
				else if (balanceType == BalanceType.Less) balance = balance.add(deposits[i].stakedLess);
				else balance = balance.add(deposits[i].stakedLess).add(getLpInLess(deposits[i].stakedLp));
			}
		}
	}
	...
	function removeStake(address staker, uint256 index) internal {
		...
		if (stakeList[staker].length > 1) {
			...
			stakeList[staker][index] = stakeList[staker][stakeList[staker].length];
			stakeList[staker].pop();
		}
	}
}